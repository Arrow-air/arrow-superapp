...
import { signed, positionTitle, timeAgo } from '../lib/format';
...
const promoting = ref<Position | null>(null);
const rationale = ref('');
...
const rankedPositions = computed(() => {
  ...
  const promotedId = bundle.value.thread.promotion?.positionId;
  return bundle.value.positions
    .map((position) => ({ position, tally: byId.get(position.id)! }))
    .sort((a, b) => {
      if (a.position.id === promotedId) return -1;
      if (b.position.id === promotedId) return 1;
...
const picksBy = (rank: 'rawRank' | 'weightedRank', score: 'rawScore' | 'weightedScore'): Pick[] =>
  rankedPositions.value
    ...
    .map(({ position, tally }) => ({
      ...
      author: memberById.value.get(position.authorId)?.handle ?? 'unknown',
      score: tally[score],
    }));
...
function startPromote(position: Position) {
  promoting.value = position;
  rationale.value = '';
  promoteError.value = '';
}
...
async function confirmPromote() {
  ...
  const position = promoting.value;
  const positionAuthor = memberById.value.get(position.authorId);
  if (!positionAuthor) {
    promoteError.value = 'Cannot find the author of this position.';
    return;
  }
  try {
    const promotion = promote({
      thread: bundle.value.thread,
      position,
      positionAuthor,
      lead: state.me,
    ...
    if (await act(() => backend.recordPromotion({ threadId: props.id, promotion }))) promoting.value = null;
  } catch (e) {
...
async function copyBounty() {
  const md = bundle.value?.thread.promotion?.bountyMarkdown;
  if (!md) return;
  try {
...
const issueUrl = computed(() => {
  const thread = bundle.value?.thread;
  if (!thread?.promotion) return '';
  const q = new URLSearchParams({ title: `Bounty: ${thread.title}`, body: thread.promotion.bountyMarkdown });
...
<template>
  ...
  <div v-else-if="!bundle || !analysis">
    ...
    <RouterLink to="/">Back to threads</RouterLink>
  </div>

  <div v-else>
    <div class="spread" style="align-items: center">
      <p class="small" style="margin: 0"><RouterLink to="/">← All threads</RouterLink></p>
    ...
    <p class="small muted" style="margin-top: 0">
      Posted by @{{ memberById.get(bundle.thread.authorId)?.handle ?? 'unknown' }}, {{ timeAgo(bundle.thread.createdAt) }}
    </p>
    ...
    <div class="layout-2" style="margin-top: 20px">
      <div class="stack">
        ...
        <!-- Promotion result -->
        <div v-if="bundle.thread.promotion" class="card stack">
          <div class="spread">
            <div>
              <div class="label">Promoted to bounty <DiscussPin anchor="bounty" class="pin-inline" /></div>
              <div style="margin-top: 4px">
                by @{{ memberById.get(bundle.thread.promotion.byMemberId)?.handle }} · weighted rank
                <b>{{ bundle.thread.promotion.weightedRankAtPromotion }}</b> · raw rank
                <b>{{ bundle.thread.promotion.rawRankAtPromotion }}</b>
              </div>
            </div>
            <div class="row">
          ...
          <div v-if="bundle.thread.promotion.overrideRationale" class="signal signal-neutral">
            <span class="label">Lead's rationale for not picking the top weighted position</span>
            <div style="margin-top: 4px">{{ bundle.thread.promotion.overrideRationale }}</div>
          </div>
          <div class="bounty-md">{{ bundle.thread.promotion.bountyMarkdown }}</div>
        </div>
        ...
        <TransitionGroup name="rank" tag="div" class="position-list">
          <article
            ...
            :key="position.id"
            class="position"
            :class="{ 'is-promoted': bundle.thread.promotion?.positionId === position.id, 'is-flash': flashPosition === position.id }"
          >
            ...
            <div class="position-main">
              <div class="position-meta">
                <strong>@{{ memberById.get(position.authorId)?.handle ?? 'unknown' }}</strong>
                <span class="muted">{{ timeAgo(position.createdAt) }}</span>
                <span v-if="bundle.thread.promotion?.positionId === position.id" class="chip chip-bounty">promoted</span>
                <span
              ...
              <div class="position-foot">
                <div class="spread" style="align-items: center">
                  ...
                  <button v-if="iAmLead && isOpen" class="btn btn-ghost" @click="startPromote(position)">Promote to bounty</button>
                </div>
                ...
                <!-- Promote panel -->
                <div v-if="promoting?.id === position.id" class="card card-subtle stack" style="margin-top: 12px">
                  <div>
                    <DiscussPin anchor="lead-override" class="pin-right" />
                    <strong>Promote this position to a bounty?</strong>
                    <div class="small muted">This closes the thread to new positions and votes.</div>
                  </div>
                  ...
                  <div v-else class="small" style="color: var(--status-success-text)">This is the top weighted position. No rationale needed.</div>
                ...
                <div class="comments">
                  ...
                  <form v-if="state.me" class="comment-form" @submit.prevent="submitComment(position.id)">
                    <input v-model="commentDraft[position.id]" type="text" placeholder="Add a comment" :aria-label="`Comment on position by ${memberById.get(position.authorId)?.handle}`" />
